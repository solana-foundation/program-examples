...
/// Emitted when a buyer opens a pull (the commit phase). Carries the fixed VRF
/// input `alpha` so verifiers can reproduce the reveal.
#[repr(C, packed)]
...
impl PullRequestedEvent {
    ...
    pub fn new(pool: Address, buyer: Address, index: u64, alpha: [u8; 32]) -> Self {
        Self { pool, buyer, index, alpha }
    }
}
