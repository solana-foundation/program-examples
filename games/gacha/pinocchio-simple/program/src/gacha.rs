...
//! TypeScript client (`selectTier` / `pullAlpha` in `@solana/gacha-simple`), so
//! anyone can reproduce a pull result from on-chain data and check it against
//! the recorded tier.
//!
...
/// Prize NFT symbol.
pub const NFT_SYMBOL: &str = "GACHAS";
/// Prize NFT metadata URI.
pub const NFT_URI: &str = "https://example.com/gacha.json";

/// `additional_metadata` key for the tier's rarity label.
pub const METADATA_RARITY_KEY: &str = "rarity";
/// `additional_metadata` key for the pull address (lowercase hex).
pub const METADATA_PULL_KEY: &str = "pull";
/// `additional_metadata` key for the buyer's entropy (lowercase hex).
pub const METADATA_CLIENT_SEED_KEY: &str = "client_seed";
/// `additional_metadata` key for the ECVRF output (lowercase hex).
pub const METADATA_BETA_KEY: &str = "beta";
/// `additional_metadata` key for the 80-byte ECVRF proof (lowercase hex).
pub const METADATA_PROOF_KEY: &str = "proof";
...
/// Encodes `bytes` as lowercase hex into `buf`, returning the used prefix of the
/// buffer. Used to write reveal provenance into prize NFT metadata without
/// `alloc`. `buf` must hold at least `2 * bytes.len()` bytes.
pub fn format_hex<'a>(bytes: &[u8], buf: &'a mut [u8]) -> &'a str {
    const HEX: &[u8; 16] = b"0123456789abcdef";
    for (i, &byte) in bytes.iter().enumerate() {
        buf[2 * i] = HEX[(byte >> 4) as usize];
        buf[2 * i + 1] = HEX[(byte & 0x0f) as usize];
    }
    // Hex digits are ASCII, so the slice is always valid UTF-8.
    unsafe { core::str::from_utf8_unchecked(&buf[..bytes.len() * 2]) }
}
