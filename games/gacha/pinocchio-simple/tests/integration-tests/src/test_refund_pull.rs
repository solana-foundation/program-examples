...
fn settled_pull_cannot_refund() {
    ...
    let (result, pull, _) = buy_pull(&mut svm, &admin.pubkey(), &buyer);
    result.assert_ok();
    settle_and_distribute(&mut svm, &admin.pubkey(), &operator, &pull, &buyer.pubkey(), &beta_from(0), &[0u8; 80])
        .assert_ok();

    let requested_slot = 1;
    svm.warp_to_slot(requested_slot + SETTLE_DEADLINE + 2);
    refund_pull(&mut svm, &admin.pubkey(), &buyer, &pull).assert_err(GachaError::NotProgramOwned);
}
