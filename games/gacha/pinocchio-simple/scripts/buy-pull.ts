...
import { findPullPda, gachaSimpleProgram } from '@solana/gacha-simple';
...
async function main(): Promise<void> {
    ...
    const [pool] = await client.gachaSimple.pdas.pool({ admin });
    const [vault] = await client.gachaSimple.pdas.vault({ admin });

    const poolAccount = await client.gachaSimple.accounts.pool.fetch(pool);
