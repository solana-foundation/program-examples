...
test('decodeHexField round-trips and rejects malformed values', () => {
    assert.deepEqual(decodeHexField('00abff1c'), new Uint8Array([0x00, 0xab, 0xff, 0x1c]));
    assert.deepEqual(decodeHexField(''), new Uint8Array(0));
    assert.throws(() => decodeHexField('abc')); // odd length
    assert.throws(() => decodeHexField('zz')); // non-hex
    assert.throws(() => decodeHexField('AB')); // uppercase — the program writes lowercase
});

// The self-certifying-NFT guarantee: the metadata a prize mint carries is enough
// to verify the whole reveal off-chain, with no transaction-history lookup.
test('verifyPrizeProvenance accepts an honest reveal and rejects tampering', () => {
    const { sk, pk } = generateKeyPair();
    const weights = [70, 25, 5];
    const pullBytes = new Uint8Array(32).fill(1);
    const clientSeed = new Uint8Array(32).fill(2);

    // '4vJ9…' is the base58 form of 32 bytes of 0x01 — the same pull as `pullBytes`.
    const alpha = pullAlpha('4vJ9JU1bJJE96FWSJKvHsmmFADCg4gpZQff4P3bkLKi' as Address, clientSeed);
    const { proof, beta } = provePull(sk, alpha);
    const tier = selectTier(beta, weights, 3);
    ...
    const provenance = {
        ...
        proof,
        pull: pullBytes,
        rarity: RARITY_LABELS[tier]!,
    };
    assert.equal(verifyPrizeProvenance(provenance, pk, weights, 3), true);

    // A mislabeled rarity is caught.
    const wrongRarity = { ...provenance, rarity: RARITY_LABELS[(tier + 1) % 3]! };
    assert.equal(verifyPrizeProvenance(wrongRarity, pk, weights, 3), false);

    // A swapped beta (operator lying about the output) is caught.
    const wrongBeta = { ...provenance, beta: new Uint8Array(64).fill(9) };
    assert.equal(verifyPrizeProvenance(wrongBeta, pk, weights, 3), false);

    // A tampered proof is caught.
    const tamperedProof = proof.slice();
    tamperedProof[0] ^= 0x01;
    assert.equal(verifyPrizeProvenance({ ...provenance, proof: tamperedProof }, pk, weights, 3), false);
