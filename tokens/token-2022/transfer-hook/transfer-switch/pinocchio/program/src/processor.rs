...
use crate::instructions::{configure_admin, initialize, initialize_extra_account_meta_list, switch, transfer_hook};
...
/// This example's own instructions, which are not part of the interface, so
/// their discriminators are single bytes we pick. They cannot collide with the
/// two above, which start with 105 and 43.
const INITIALIZE: u8 = 0;
const CONFIGURE_ADMIN: u8 = 1;
const SWITCH: u8 = 2;
...
pub fn process_instruction(
    ...
    match instruction_data {
        ...
        [CONFIGURE_ADMIN, ..] => {
            log!("Instruction: ConfigureAdmin");
            configure_admin(program_id, accounts)
        }
        [SWITCH, data @ ..] => {
            log!("Instruction: Switch");
            switch(program_id, accounts, data)
        }
