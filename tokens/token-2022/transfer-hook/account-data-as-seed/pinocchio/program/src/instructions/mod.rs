mod initialize;
mod initialize_extra_account_meta_list;
mod transfer_hook;

pub use initialize::*;
pub use initialize_extra_account_meta_list::*;
pub use transfer_hook::*;

/// The SPL Token-2022 program ID
/// (`TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`).
///
/// Unlike the legacy SPL Token program (which `pinocchio-token` wraps), there is
/// no pinocchio crate for Token-2022, so its instructions are built by hand and
/// CPI'd into this program.
pub const TOKEN_2022_PROGRAM_ID: pinocchio::Address =
    pinocchio::Address::from_str_const("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb");

/// Seed prefix for the `ExtraAccountMetaList` PDA.
///
/// Fixed by the transfer-hook interface: during a transfer Token-2022 derives
/// `[b"extra-account-metas", mint]` against the hook program to discover which
/// additional accounts the hook needs, so the account must live at exactly this
/// address to be found.
pub const EXTRA_ACCOUNT_METAS_SEED: &[u8] = b"extra-account-metas";

/// First seed of the counter PDA; the second is the token owner.
///
/// This is what the example is named for. The counter is per-owner, and the
/// owner is not passed in — Token-2022 reads it straight out of the source
/// token account's data (bytes 32..64) using the `AccountData` seed recorded in
/// the `ExtraAccountMetaList`, derives the PDA, and passes the result to
/// `Execute`.
pub const COUNTER_SEED: &[u8] = b"counter";

/// Where a token account stores its owner: the 32 bytes after the mint.
///
/// The `ExtraAccountMetaList` names this same range, so the program and
/// Token-2022's resolver must agree on it or they derive different counters.
pub const TOKEN_ACCOUNT_OWNER_RANGE: core::ops::Range<usize> = 32..64;

/// Size of the counter account: a single little-endian `u64`.
///
/// The Anchor version reserves 16 bytes because it stores an 8-byte account
/// discriminator ahead of the value; there is no such discriminator here.
pub const COUNTER_SIZE: usize = 8;
