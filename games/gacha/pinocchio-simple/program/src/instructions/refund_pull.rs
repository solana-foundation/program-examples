...
use crate::{
    ...
    state::{common::find_vault_pda, pool::Pool, pull::Pull},
    GachaError,
};
...
/// delay a payout, but can never keep the buyer's funds. A pull account existing
/// means the pull is still pending — settling closes it — and a refunded pull
/// can never be re-created: pull PDAs are seeded by a monotonic pool index.
