...
/// Seed of the counter PDA.
///
/// This is the one extra account the hook resolves. It is listed in the
/// `ExtraAccountMetaList` as a literal seed, so Token-2022 derives the same
/// address on-chain and passes it to `Execute` without the client having to
/// name it explicitly.
