 ...
 * RPC. It mirrors what happens around a real `buyPull` -> `settleAndDistribute`:
 *
 *   1. A pool records an operator (an Ed25519 key that is also its ECVRF key).
 ...
 *   3. The operator proves `beta = VRF(alpha)` and submits it via
 *      `settleAndDistribute`, which mints the prize NFT carrying the full reveal
 *      provenance in its metadata.
 *   4. Anyone recomputes `alpha` from the NFT's metadata, verifies the proof,
 *      and reproduces the selected tier — no transaction history needed.
 *
 * Run: `just demo`
 */
...
import { generateKeyPair, provePull, publicKeyFromSeed, pullAlpha, selectTier, verifyPull } from '@solana/gacha-simple';
import { findPullPda, RARITY_LABELS, verifyPrizeProvenance } from '@solana/gacha-simple';
import { GACHA_SIMPLE_PROGRAM_ADDRESS } from '@solana/gacha-simple';
...
async function main() {
    ...
    const buyer = GACHA_SIMPLE_PROGRAM_ADDRESS;
    ...
    // The same check driven purely from what the prize NFT's metadata carries.
    const provenance = {
        beta,
        clientSeed,
        proof,
        pull: new Uint8Array(getAddressEncoder().encode(pull)),
        rarity: RARITY_LABELS[tier]!,
    };
    const selfCertified = verifyPrizeProvenance(provenance, pk, weights, weights.length);
    console.log(`NFT metadata self-certifies: ${selfCertified}`);

    if (!verified || !selfCertified) process.exit(1);
}
