...
///
/// Holds the fixed entry fee, the registered off-chain VRF `operator`, the fixed
/// tier `weights`, a monotonic `pulls_count` used to derive unique pull
/// accounts, and `pending_pulls` tracking outstanding refund liabilities. One
/// pool per admin wallet.
///
...
pub struct Pool {
    ...
    /// as the ECVRF public key verifiers check reveals against — it is fixed at
    /// init, so the key can never be swapped mid-pool.
