...
pub struct InitPoolData {
    ...
    /// Doubles as the ECVRF public key verifiers check reveals against.
...
pub fn process(accounts: &mut [AccountView], data: &InitPoolData) -> ProgramResult {
    ...
    // operator actually controls the ECVRF secret is proven by every reveal
    // verifying off-chain against this key.
    ...
    let event =
        PoolInitializedEvent::new(*accounts.admin.address(), operator, entry_fee, settle_deadline_slots, tier_count);
