...
pub fn setup() -> (LiteSVM, Keypair) {
    ...
    let so_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/deploy/gacha_simple_program.so");
...
/// Serializes `SettleAndDistribute` instruction data. The program never verifies
/// the ECVRF proof on-chain, so tests may pass arbitrary bytes.
pub fn settle_data(beta: &[u8; 64], proof: &[u8; 80]) -> Vec<u8> {
...
pub fn settle_metas(admin: &Address, operator: &Address, pull: &Address, buyer: &Address) -> Vec<AccountMeta> {
    let (pool, _) = client::Pool::find_pda(admin);
...
pub fn settle_and_distribute(
    ...
    let accounts = settle_metas(admin, &operator.pubkey(), pull, buyer);
    let ix = Instruction { program_id: PROGRAM_ID, accounts, data: settle_data(beta, proof) };
