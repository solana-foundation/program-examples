...
/// to. Here that is one account: the counter, which Token-2022 derives from the
/// literal seed recorded in the list and appends itself.
///
/// Accounts:
...
///   5. `[writable]` counter (PDA `[b"counter"]`)
///
...
/// the new count, which is the behaviour the example is named for.
