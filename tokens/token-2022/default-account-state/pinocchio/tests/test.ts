...
// The DefaultAccountState TLV entry is type (2) + length (2) + value (1).
const EXTENDED_MINT_SIZE = 171;
...
const PROGRAM_SO = path.join(
    ...
    'tests',
    'fixtures',
    'token_2022_default_account_state_pinocchio_program.so',
);

describe('Token-2022 Default Account State (Pinocchio)', () => {
    let svm: LiteSVM;
    ...
    it('Creates a Token-2022 mint with a default account state', async () => {
        const decimals = 9;
        ...
        const ix = {
            ...
            accounts: [
                ...
                // as the payer so a single signature can both fund the account and, as the
                // freeze authority, sign the post-init default-state update.
        ...
        const defaultStateExtension = extensions.find(e => e.__kind === 'DefaultAccountState');
        if (defaultStateExtension?.__kind !== 'DefaultAccountState') {
            throw new Error('DefaultAccountState extension not found on the mint');
        }

        // The program initializes the default state to `Frozen`, then updates it to
        // `Initialized` — so the final stored default is `Initialized`.
        assert.equal(defaultStateExtension.state, AccountState.Initialized);
