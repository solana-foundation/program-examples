...
use crate::{
    gacha::{derive_alpha, format_hex, format_u64, select_tier, MAX_TIERS},
    GachaError,
};
...
fn formats_hex_lowercase() {
    let mut buf = [0u8; 8];
    assert_eq!(format_hex(&[0x00, 0xab, 0xff, 0x1c], &mut buf), "00abff1c");
    let mut buf = [0u8; 4];
    assert_eq!(format_hex(&[], &mut buf), "");
}

#[test]
fn formats_hex_proof_sized_input() {
    let mut bytes = [0u8; 80];
    for (i, byte) in bytes.iter_mut().enumerate() {
        *byte = i as u8;
    }
    let mut buf = [0u8; 160];
    let hex = format_hex(&bytes, &mut buf);
    assert_eq!(hex.len(), 160);
    assert!(hex.starts_with("000102030405"));
    assert!(hex.ends_with("4d4e4f"));
}
