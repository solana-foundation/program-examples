...
/// ```
///
/// The seed config is `spl-tlv-account-resolution`'s packed form of two seeds,
/// and is the whole point of this example:
///
/// - `Seed::Literal { bytes: b"counter" }` → a `1` tag, the byte length, then
///   the bytes.
/// - `Seed::AccountData { account_index: 0, data_index: 32, length: 32 }` → a
///   `4` tag followed by those three bytes. It tells Token-2022 to take 32
///   bytes at offset 32 of account 0 — the source token account's owner field —
///   and use them as a seed.
///
/// So the counter is per-owner, and no caller ever names it: Token-2022 reads
/// the owner out of the token account itself, derives `[b"counter", owner]`
/// against this program, and passes the result to `Execute`.
///
...
///   2. `[]`                 mint
///   3. `[writable]`         counter (PDA `[b"counter", payer]`)
...
pub fn initialize_extra_account_meta_list(program_id: &Address, accounts: &mut [AccountView]) -> ProgramResult {
    ...
    // The counter is per token owner. Here that is the payer, who is creating
    // their own counter ahead of transferring; during a transfer the same
    // address is rederived from the source account's owner field. The two only
    // agree when the payer owns the source token account — which is the setup
    // this example (and the Anchor version) demonstrates.
    let (counter_address, counter_bump) =
        Address::find_program_address(&[COUNTER_SEED, payer.address().as_ref()], program_id);
